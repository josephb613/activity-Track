...
//     async fetchContent() {
//         const browser = await puppeteer.launch({ headless: true, ignoreHTTPSErrors: true }); // Disable SSL verification
//         try {
...
//                 await this.notifyUser(significantChanges);
...
//             return prefix + change.value.split('\n').slice(0, 5).join('\n');
//         }).join('\n');
...
// const cronSchedule = process.env.CRON_SCHEDULE || '*/15 * * * *'; // Toutes les heures par défaut
// const emailConfig = {
...
class WebUpdateTracker {
    ...
    async fetchContent() {
        ...
        try {
            ...
            // Attendre que les frameworks JavaScript aient terminé leur exécution
            await page.waitForTimeout(5000); // ajuster selon les besoins
